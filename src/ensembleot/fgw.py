...
Stage 12a restriction: X and Y must share the same feature dimension
(``X.shape[1] == Y.shape[1]``). A generalized FGW across different feature
spaces is left to a later stage. Full sample × sample cost / transport
...
def _single_run(
    ...
    # intra-domain structure costs (K_x×K_x, K_y×K_y)
    ...
    M = ot.dist(centers_x, centers_y, metric=metric)
...
def run_ensemble_fgw(
    ...
    Stage 12a restriction: requires ``X.shape[1] == Y.shape[1]``.
    """
    if n_runs < 1:
    ...
    if X.shape[1] != Y.shape[1]:
        raise ValueError(
            "run_ensemble_fgw (Stage 12a) requires X and Y to share feature "
